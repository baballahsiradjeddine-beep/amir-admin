...
export function updateTransaction(transactionId: string, updates: Partial<Transaction>): Transaction {
  const db = getDatabase();
  const snakeUpdates = toSnakeCase(updates);
  const now = getCurrentTimestamp();
...
export function updateCurrencyTransaction(transactionId: string, updates: Partial<CurrencyTransaction>): CurrencyTransaction {
  const db = getDatabase();
  const snakeUpdates = toSnakeCase(updates);
  const now = getCurrentTimestamp();
  ...
  db.prepare(`UPDATE currency_transactions SET ${fields.join(', ')} WHERE id = ?`).run(...values);

  return getCurrencyTransactionById(transactionId)!;
}
